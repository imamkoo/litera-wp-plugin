...
function my_react_plugin_enqueue_scripts() {
    if (is_single()) {
        // Fix: Load the local bundle.js instead of literaa.xyz CDN
        wp_enqueue_script('my-react-plugin-script', plugin_dir_url(__FILE__) . 'bundle.js', [], filemtime(plugin_dir_path(__FILE__) . 'bundle.js'), true);
...
function my_react_plugin_add_button_after_post_content($content) {
    if (is_single() && strpos($content, '[litera_widget]') === false) {
